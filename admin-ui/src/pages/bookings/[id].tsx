...
class EditBooking extends React.Component<Props, State> {
  ...
  languages: string[] = ["admin@seatsurfing.local", "admin@seasdfasdf"];
  ...
  loadSpaces = async (
    ...
    console.log(
      "Loading spaces " + enter.toString() + " --> " + leave.toString()
    );
    ...
    return OrgSettings.list().then((settings) => {
      settings.forEach((s) => {
        ...
        if (s.name === "limit_booking_to_24_hours") {
          this.limitBookingTo24Hours = s.value === "1";
        }
        // this.setState({ loading: false });
      });
    });
  ...
  updateCanSearch = async () => {
    ...
    if (this.limitBookingTo24Hours && bookingDurationHours > 24) {
      res = false;
      hint = this.props.t("errorBookingMoreThan24Hours");
    }
    let self = this;
    return new Promise<void>(function (resolve, reject) {
      self.setState(
