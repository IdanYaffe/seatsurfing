...
class Settings extends React.Component<Props, State> {
    ...
    return OrgSettings.list().then((settings) => {
      ...
      settings.forEach((s) => {
        ...
        if (s.name === "subscription_active")
          state.subscriptionActive = s.value === "1";
        if (s.name === "subscription_max_users")
          state.subscriptionMaxUsers = window.parseInt(s.value);
        ...
        if (s.name === "limit_booking_to_24_hours")
          state.limitBookingTo24Hours = s.value === "1";
      });
  ...
  onSubmit = (e: any) => {
    ...
    let payload = [
      ...
      new OrgSettings(
        "limit_booking_to_24_hours",
        this.state.limitBookingTo24Hours ? "1" : "0"
      ),
    ];
        ...
        <Form onSubmit={this.onSubmit} id="form">
          ...
          <Form.Group as={Row}>
            <Col sm="6">
              <Form.Check
                type="switch"
                id="limit_booking_to_24_hours"
                label={this.props.t("limitBookingTo24Hours")}
                checked={this.state.limitBookingTo24Hours}
                onChange={(e: any) =>
                  this.setState({ limitBookingTo24Hours: e.target.checked })
                }
              />
            </Col>
          </Form.Group>
